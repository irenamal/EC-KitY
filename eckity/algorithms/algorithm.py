        ...
        for gen in range(self.max_generation):
            ...
            if STATISTICS and 0 == self.generation_num % 5:
                ...
                for num, operator in enumerate(operators):
                    ...
                    for ind, fitness_scores in zip(best_ind_copies[num * REPS:(num + 1) * REPS], eval_results_best):
                        best_after_fitness.append(fitness_scores[2])
                    eval_results_rand = self.executor.map(self.population.sub_populations[0].evaluator._evaluate_individual,
                                                          rand_ind_copies[num * REPS:(num + 1) * REPS])

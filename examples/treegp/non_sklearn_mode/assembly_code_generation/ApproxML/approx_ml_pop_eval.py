...
class ApproxMLPopulationEvaluator(PopulationEvaluator):
    ...
    def gp_to_ga_representation(self, gp_individual):
        # score, lifetime, written bytes, writing rate, size
        parameters = [gp_individual.fitness_parts[0], gp_individual.fitness_parts[1], gp_individual.fitness_parts[2],
                      gp_individual.fitness_parts[3], gp_individual.size()]
        ga_individual = FloatVector(fitness=0, length=len(parameters))
        ga_individual.set_vector(parameters)
        return ga_individual.get_vector()

    @override
    def _evaluate(self, population: Population, gen=0) -> Individual:
        """
        ...
        for i, sub_population in enumerate(population.sub_populations):
            if self.is_approx:
                ...
                fitness = self._evaluate_individuals(
                    sub_population.individuals,
                    sub_population.evaluator,
                    i
                )
                ...
                fitness_data = []
                for ind, fitness_scores in zip(sub_population.individuals, fitness):
                    ind.set_evaluation(fitness_scores[0], fitness_scores[1], fitness_scores[2], fitness_scores[3])
                    fitness_data.append(fitness_scores[2])

                best_of_gen_candidates = sub_population.individuals

                # Update population dataset
                vecs = [self.gp_to_ga_representation(ind) for ind in sub_population.individuals]
                self._update_dataset(vecs, fitness_data)
        ...
        # Update the best individual of the current generation
    ...
    def _approximate_individuals(self,
        ...
        if self.gen > 0 and self.gen % self.gen_sample_step == 0:
            ...
            if sample_size > 0:
                ...
                vectors = [self.gp_to_ga_representation(ind) for ind in sample_inds]
                self._update_dataset(vectors, [fitness_score[2] for fitness_score in fitness_scores])
        ...
        for j, ind in enumerate(individuals):
            if not self.hide_fitness and ind in inds2scores:
                ind.set_evaluation(inds2scores[ind][0], inds2scores[ind][1], inds2scores[ind][2], inds2scores[ind][3])
            else:
                ind.set_evaluation(preds[j], preds[j], preds[j], [0, 0, 0, 0])
    ...
    def _get_cosine_scores(self,
        ...
        similarity_scores = [
            genotypes.apply(lambda row:
                            utils.cosine_similarity(self.gp_to_ga_representation(ind), row),
                            axis=1, raw=True
                            ).max()
    ...
    def predict(self, individuals: List[Individual]):
        ...
        """
        ind_vectors = [self.gp_to_ga_representation(ind) for ind in individuals]
    ...
    def export_dataset(self, folder_path) -> None:
        ...
        """
        self.df.to_csv(f'{folder_path}/summary.csv', index=False)
